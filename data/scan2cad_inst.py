...
class Dataset:
    def __init__(self, test=False):
        self.data_root = cfg.data_root  # dataset
        self.dataset = cfg.dataset      # scan2cad
        self.data_dir = cfg.data_dir    # data
    ...
    def load_data_split(self, split):
        data_path = os.path.join(self.data_root, self.dataset, self.data_dir)
        all_scan_names = list(set(os.path.basename(scan)[:12] \
            for scan in os.listdir(data_path) if scan.startswith('scene')))
        # split scans
        ...
        split_data_names = []
        # error scans
        error_file = os.path.join(data_path, 'error_scan.txt')
        with open(error_file, 'r') as f:
            error_scans = []
            error_scans = f.read().splitlines()
        # collect valid split scans
        for i, scan_name in enumerate(all_scan_names):
            if scan_name in error_scans: continue
            if scan_name in split_names:
                split_data_names.append(os.path.join(data_path, scan_name + self.filename_suffix))
        logger.info(f'Data found for {split}: {len(split_data_names):4d}/{len(split_names):4d}')
        return split_data_names

    def trainLoader(self):
        train_file_names = self.load_data_split('train')
        self.train_file_names = sorted(train_file_names)
        self.train_files = [torch.load(i) for i in self.train_file_names if os.path.exists(i)]
    ...
    def valLoader(self):
        val_file_names = self.load_data_split('val')
        self.val_file_names = sorted(val_file_names)
        self.val_files = [torch.load(i) for i in self.val_file_names if os.path.exists(i)]
    ...
    def testLoader(self):
        test_file_names = self.load_data_split(self.test_split)
        self.test_file_names = sorted(test_file_names)
        self.test_files = [torch.load(i) for i in self.test_file_names if os.path.exists(i)]
